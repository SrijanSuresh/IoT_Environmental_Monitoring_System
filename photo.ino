...
void connectToMQTT() {
  ...
  while (!client.connected()) {
    if (client.connect("arduinoLightServo", mqtt_user, mqtt_password)) {
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    client.publish("SrijanSuresh/feeds/lightintensity", String(lightVal).c_str());
    Serial.println("Published light intensity to Adafruit IO.");

    // Simple logic: Open vent if too bright (>700) or too dark (<300)
    if (lightVal > 700 || lightVal < 300) {
      ventServo.write(90); // Open vent
      Serial.println("Vent OPENED (Light threshold hit).");
    } else {
      ventServo.write(0); // Close vent
      Serial.println("Vent CLOSED (Normal light).");
    }
  }
}